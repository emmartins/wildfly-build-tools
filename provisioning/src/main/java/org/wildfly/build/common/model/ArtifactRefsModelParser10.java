...
public class ArtifactRefsModelParser10 {
    ...
    public String parseArtifactName(String artifactName, final Map<String, Artifact> result) throws XMLStreamException {
        return parseArtifactName(artifactName, null, result);
    }

    public String parseArtifactName(String artifactName, String defaultExtension, final Map<String, Artifact> result) throws XMLStreamException {
        if (!result.containsKey(artifactName)) {
            // there is no artifact ref yet with the parsed name
            Artifact artifact = Artifact.fromString(artifactName);
            if (artifact != null) {
                // the artifact name's itself defines an artifact ref
                if (artifact.getExtension() == null && defaultExtension != null) {
                    artifact.setExtension(defaultExtension);
                }
                result.put(artifactName, artifact);
            }
        }
        return artifactName;
    }
