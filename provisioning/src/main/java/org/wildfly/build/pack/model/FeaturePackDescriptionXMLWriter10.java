...
public class FeaturePackDescriptionXMLWriter10 {
    ...
    public void write(FeaturePackDescription featurePackDescription, File outputFile) throws XMLStreamException, IOException {
        ...
        processVersions(featurePackDescription.getVersions(), featurePackElementNode);
        processConfig(featurePackDescription.getConfig(), featurePackElementNode);
        processArtifacts(featurePackDescription.getArtifacts(), featurePackElementNode);
        processFilePermissions(featurePackDescription.getFilePermissions(), featurePackElementNode);
        Writer writer = new FileWriter(outputFile);
        try {
            XMLOutputFactory factory = XMLOutputFactory.newInstance();
            featurePackElementNode.marshall(new FormattingXMLStreamWriter(factory.createXMLStreamWriter(writer)));
        } finally {
            writer.close();
        }
    }
    ...
    protected void processVersions(Versions versions, ElementNode featurePackElementNode) {
        if (!versions.getArtifacts().isEmpty()) {
            final ElementNode versionsElementNode = new ElementNode(featurePackElementNode, Element.VERSIONS.getLocalName());
            for (Artifact artifact : versions.getArtifacts()) {
                processVersionsArtifact(artifact, versionsElementNode);
            }
            featurePackElementNode.addChild(versionsElementNode);
        }
    }

    protected void processVersionsArtifact(Artifact artifact, ElementNode versionsElementNode) {
    ...
    protected void processArtifacts(List<CopyArtifact> copyArtifacts, ElementNode featurePackElementNode) {
        if (!copyArtifacts.isEmpty()) {
            final ElementNode artifactsElementNode = new ElementNode(featurePackElementNode, Element.ARTIFACTS.getLocalName());
