...
class FeaturePackDescriptionXMLParser10 implements XMLElementReader<FeaturePackDescription> {
    ...
    enum Element {
        ...
        static {
            ...
            elementsMap.put(new QName(NAMESPACE_1_0, Element.VERSIONS.getLocalName()), Element.VERSIONS);
            ...
            elementsMap.put(new QName(NAMESPACE_1_0, Element.ARTIFACTS.getLocalName()), Element.ARTIFACTS);
    ...
    private void parseVersions(final XMLStreamReader reader, final FeaturePackDescription result) throws XMLStreamException {
        final Versions versions = result.getVersions();
        while (reader.hasNext()) {
            switch (reader.nextTag()) {
                case XMLStreamConstants.END_ELEMENT: {
                    return;
                ...
                case XMLStreamConstants.START_ELEMENT: {
                    ...
                    switch (element) {
                        case ARTIFACT:
                            versions.getArtifacts().add(parseArtifact(reader));
                            break;
                        default:
                            throw unexpectedContent(reader);
                    }
    ...
    private void parseArtifacts(final XMLStreamReader reader, final FeaturePackDescription result) throws XMLStreamException {
        while (reader.hasNext()) {
            switch (reader.nextTag()) {
                case XMLStreamConstants.END_ELEMENT: {
                    return;
    ...
    private void parseCopyArtifact(XMLStreamReader reader, FeaturePackDescription result) throws XMLStreamException {
        ...
        result.getArtifacts().add(copyArtifact);
        while (reader.hasNext()) {
            switch (reader.nextTag()) {
                case XMLStreamConstants.END_ELEMENT: {
                    return;
